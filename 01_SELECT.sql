...
/* 컬럼명 AS 별칭 : 별칠 띄어쓰기 X, 특수문자 X, 문자만 O
 * 
...
SELECT SYSDATE - 1 AS 어제, SYSDATE AS 현재시간, SYSDATE + 1 AS 내일
FROM DUAL;
...
-- DB 리터럴 : 임이로 지정한 값을 기존 테이블에 존재하는값처럼 사용하는 것
--> (필수) DB의 리터럴 표기법은 ' ' 홑따옴표
--> " " 쌍따옴표는 특수문자, 대소문자, 기호 등을 구분하여 나타날 때 사용하는 표기법
...
-- 주의사항 2) DISIINCT 구문은 SELECT 제일 앞에 작성되어야 한다.
...
--                                       (오름 / 내림차순) 
-- EMPLOYEE 테이블에서 급여가 3백만원 초과인 사원읜
-- 사번, 이름, 급여, 부서코드를 조회
...
-- EMPLOYEE 테이블에서 EMAIL의 _ 앞에 글자가 세글자인 사원만 조회
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D1', 'D6', 'D9'); -- 9AUD
/*
WHERE DEPT_CODE = 'D1'
OR DEPT_CODE = 'D6'
OR DEPT_CODE = 'D9';
...
OR DEPT_CODE IS NOT NULL; -- 부서코드 없는 2명 포함 14명
